...
import { encodeRouteToPath } from 'panaromaswap_v3sdk';
...
import { PanaromaswapMulticallProvider } from '../multicall-panaromaswap-provider';
...
/**
 * Provider for getting quotes on Panaromaswap V3.
 *
 * @export
...
export class V3QuoteProvider implements IV3QuoteProvider {
   ...
   * Only supports the Panaromaswap Multicall contract as it needs the gas limitting functionality.
  ...
  constructor(
    ...
    // Only supports Panaromaswap Multicall as it needs the gas limitting functionality.
    ...
    if (!quoterAddress) {
      throw new Error(
        `No address for Panaromaswap QuoterV2 Contract on chain id: ${chainId}`
      );
    }
