...
export type PanaromaswapMulticallConfig = {
  gasLimitPerCallOverride?: number;
};

/**
 * The PanaromaswapMulticall contract has added functionality for limiting the amount of gas
 ...
 *
 * @export
 * @class PanaromaswapMulticallProvider
 */
export class PanaromaswapMulticallProvider extends IMulticallProvider<PanaromaswapMulticallConfig> {
  private multicallContract: PanaromaswapInterfaceMulticall;
  ...
  constructor(
    ...
    if (!multicallAddress) {
      throw new Error(
        `No address for Panaromaswap Multicall Contract on chain id: ${chainId}`
      );
    }

    this.multicallContract = PanaromaswapInterfaceMulticall__factory.connect(
      multicallAddress,
      this.provider
    );
  }
