...
import { Protocol } from 'panaromaswap_routersdk';
import { MethodParameters } from 'panaromaswap_v3sdk';
...
describe('alpha router integration', () => {
  ...
  const multicall2Provider = new PanaromaswapMulticallProvider(
    ChainId.MAINNET,
    hardhat.provider
  );
  ...
  beforeAll(async () => {
    ...
    await hardhat.fund(
      ...
      [
        ...
        parseAmount('1000', BANG_MAINNET),
        parseAmount('5000000', DAI_MAINNET),
      ],
      [
        '0x4fFDD5Cf5f16D93d481CEDb70A1cBE37c3AaD329', // Binance peg tokens
      ]
    );
    ...
    await hardhat.fund(
      ...
      [
        '0x4fFDD5Cf5f16D93d481CEDb70A1cBE37c3AaD329', // WETH token
      ]
    );
    ...
    const aliceBANGBalance = await hardhat.getBalance(
      alice._address,
      BANG_MAINNET
    );
    expect(aliceBANGBalance).toEqual(parseAmount('1000', BANG_MAINNET));
...
describe('quote for other networks', () => {
  ...
  for (const chain of _.filter(
    ...
    (c) =>
      ...
      // Tests are failing https://github.com/Panaromaswap/smart-order-router/issues/104
